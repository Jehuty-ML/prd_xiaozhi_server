"""Phase-4 smoke: hello + listen/detect through TTS pipeline + optional abort."""
...
async def run(url: str, text: str, timeout: float, abort: bool) -> int:
    ...
    async with websockets.connect(
        ...
        client_hello = {
            ...
            or {"format": "opus", "sample_rate": 16000},
        }
        ...
        while asyncio.get_event_loop().time() < deadline:
            ...
            if isinstance(msg, bytes):
                ...
                if data.get("type") == "tts":
                    ...
                    if state == "stop":
                        got_stop = True
            if got_tts_state and got_audio and got_stop:
                break

        if not got_tts_state:
            print("SMOKE FAIL: no TTS state frame within timeout", file=sys.stderr)
            return 1
        if not got_audio:
            print("SMOKE FAIL: no Opus/audio bytes within timeout", file=sys.stderr)
            return 1
        ...
        if abort:
            ...
            while asyncio.get_event_loop().time() < deadline:
                ...
                print(
                    "<<",
                    stop if not isinstance(stop, bytes) else f"bytes[{len(stop)}]",
                )
                if isinstance(stop, str):
                    try:
                        ...
                        continue
                    if (
                        stop_obj.get("type") == "tts"
                        and stop_obj.get("state") == "stop"
                    ):
                        abort_stop = True
                        break
            if not abort_stop and not got_stop:
                print("SMOKE FAIL: abort did not yield tts stop", file=sys.stderr)
                return 1

        print("SMOKE OK: protocol hello + TTS state/audio downlink")
        return 0
