...
安装完后，你需要为这个项目找一个安放配置文件的目录，例如我们可以新建一个文件夹叫`xiaozhi-microserver`。

创建好目录后，你需要在`xiaozhi-microserver`下面创建`data`文件夹和`models`文件夹，`models`下面还要再创建`SenseVoiceSmall`文件夹。
...
用浏览器打开[这个链接](../main/xiaozhi-microserver/docker-compose_all.yml)。
...
用浏览器打开[这个链接](../main/xiaozhi-microserver/xiaozhi-control-admin/config_from_api.yaml)。
...
## 3. 清除历史版本镜像和容器
接下来打开命令行工具，使用`终端`或`命令行`工具 进入到你的`xiaozhi-microserver`，执行以下命令
...
复制`参数值`后，打开`xiaozhi-microserver`下的`data`目录的`.config.yaml`文件。此刻你的配置文件内容应该是这样的：
...
你需要把它重命名成`xiaozhi-esp32-server`，在这个文件里，进入到`main`文件夹，再进入到`xiaozhi-microserver`，好了请记住这个目录`xiaozhi-microserver`。

```
# 继续使用conda环境
conda activate xiaozhi-esp32-server
# 进入到你的项目根目录，再进入main/xiaozhi-microserver
...
如果你的`xiaozhi-microserver`目录没有`data`，你需要创建`data`目录。
如果你的`data`下面没有`.config.yaml`文件，你可以把`xiaozhi-microserver`目录下的`config_from_api.yaml`文件复制到`data`，并重命名为`.config.yaml`
...
```
# 确保在xiaozhi-microserver目录下执行
conda activate xiaozhi-esp32-server
bash start_dev_services.sh  # Windows: start_dev_services.bat
```
