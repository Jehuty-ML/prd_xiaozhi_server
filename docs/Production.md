...
**第六期起主运行时为** [`main/xiaozhi-microserver`](../main/xiaozhi-microserver)（六微服务）。  
历史单体 [`main/xiaozhi-server`](../main/xiaozhi-server) 已退役（见其 `RETIRED.md`）；存量 Docker `server_*` 镜像仍可构建，新部署请走 microserver。

智控台 `manager-api` / `manager-web` 继续保留，由 `xiaozhi-model-admin` 对接。

安装步骤见 [Deployment.md](./Deployment.md) / [Deployment_all.md](./Deployment_all.md)（文档中的 Server 路径逐步切到 microserver）。  
...
| 8 | 指标 | 抓取 access `GET :8103/metrics` 与 admin `GET :8004/metrics` |
...
| 服务 | 路径 | 用途 | 成功 | 失败 |
|------|------|------|------|------|
| access | `:8103/health` | liveness | `200` `ok` | 进程无响应 |
| access | `:8103/ready` | readiness | `200` ready | `503` 连接打满 |
| model-admin | `:8004/health` | liveness | `200` `ok` | 进程无响应 |
| model-admin | `:8004/ready` | readiness | `200` ready | `503` access 打满 |

```bash
curl -sS http://127.0.0.1:8103/health
curl -sS http://127.0.0.1:8103/ready
...
旧路径 `cd main/xiaozhi-server && docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d` 仍可用于存量环境，但**不再作为文档与 CI 主线**。
